...
/**
 * Where a student lands after a resource-link launch. In a real tool this is
 * the activity itself; here it shows the item id, a button that posts a
 * completion grade back to the LMS gradebook via AGS, and a button that
 * fetches the course roster via NRPS.
 */
export function LaunchedClient() {
  const params = useSearchParams()
  const item = params.get('item') ?? '(none)'
  ...
  // Prove a cookieless round-trip. Write a value into the platform's storage
  // frame and read it back — this survives even when the browser blocks
